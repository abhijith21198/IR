...
for title in df['Name of Publication']:
    ...
    # Removing punctuations There are 14 punctuation marks that are used in the English language. They are: the period, question mark, exclamation point, comma, colon, semicolon, dash, hyphen, brackets, braces, parentheses, apostrophe, quotation mark, and ellipsis
    clean_doc = re.sub(r'[%s]' % re.escape(string.punctuation), '', clean_doc)
    #removing stop words. What are Stopwords in English?
...
stem_words=[]
for word in token_words:
    #Stemming and removing stop words like articles/proposition/conjunction and punctuations like comma/colon/hash.
...
if(boolean):
    ...
    #Inverted index is very useful for search engine query processing as it allows us to find docs that word occurs in.
